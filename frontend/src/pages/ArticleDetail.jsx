...
<section className="relative overflow-hidden pt-32 pb-16 bg-gradient-to-r from-blue-950 via-slate-900 to-slate-700">
  ...
  <div className="container-custom relative z-10 text-white">
    <div className="max-w-3xl">
      ...
      {/* Article Content */}
      <section className={`py-16 transition-colors duration-300 ${isDark ? 'bg-dark-900' : 'bg-white'}`}>
        <div className="container-custom">
          <div className="max-w-3xl mx-auto">
            <img
              src={article.image}
              alt={article.title}
              className={`w-full rounded-xl mb-8 transition-colors duration-300 ${isDark ? 'shadow-lg shadow-dark-800' : 'shadow-md'}`}
            />
            ...
            {article.tags && article.tags.length > 0 && (
              <div className={`mt-8 pt-8 border-t transition-colors duration-300 ${isDark ? 'border-dark-700' : 'border-gray-200'}`}>
                <h4 className={`font-semibold mb-3 ${isDark ? 'text-white' : 'text-gray-800'}`}>Tags:</h4>
                <div className="flex flex-wrap gap-2">
                  {article.tags.map((tag, idx) => (
                    <span key={idx} className={`px-3 py-1 text-sm rounded-full transition-colors duration-300 ${isDark ? 'bg-dark-700 text-gray-300' : 'bg-gray-100 text-gray-600'
                      }`}>
                      #{tag}
                    </span>
                  ))}
            ...
            <div className={`mt-8 pt-8 border-t transition-colors duration-300 ${isDark ? 'border-dark-700' : 'border-gray-200'}`}>
              ...
              <div className="flex gap-3">
                <button className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${isDark ? 'bg-dark-700 text-gray-300 hover:bg-dark-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}>
                  📤 Share
                </button>
                <button className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${isDark ? 'bg-dark-700 text-gray-300 hover:bg-dark-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
      ...
      <section className={`py-16 transition-colors duration-300 ${isDark ? 'bg-dark-800' : 'bg-gray-50'}`}>
        <div className="container-custom">
            ...
            {allArticles
              ...
              .map((related) => (
                <Link
                  ...
                  className={`group rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 ${isDark ? 'bg-dark-900 hover:shadow-dark-800' : 'bg-white'
                    }`}
                >
                  ...
                  <div className="p-5">
                    <h3 className={`font-bold mb-2 line-clamp-2 transition-colors ${isDark ? 'text-white group-hover:text-yellow-400' : 'text-gray-900 group-hover:text-yellow-500'
                      }`}>
                      {related.title}
                    </h3>
